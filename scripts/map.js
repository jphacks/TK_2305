...
getPins().then((pins) => {
  pins.forEach((pin) => {
    ...
    marker.bindPopup(
      `<div class="custom-title">${pin.forgotten_item}</div><div class="popup-content">ユーザー名: ${pin.user_name}<br>お礼: ${pin.reward}円<br>いつまで: ${timeDiff}分以内<br><p><button id="detail-button" class="orange-round-button">詳細情報</button></p>`,
      // {maxWidth: "auto"},
